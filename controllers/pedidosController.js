...
export const getPedidoById = async (req, res) => {
  ...
  try {
    ...
    res.json({ status: 'OK', data: { ...pedido[0], productos: productosConIngredientes } })
  } catch (error) {
    res.status(500).json({ status: 'ERROR', message: `Error al obtener detalle del pedido ${id}`, error: error.message })
  }
}
...
export const getPedidosByEstado = async (req, res) => {
  ...
  try {
    const pedidos = await sql`
      SELECT * FROM pedidos WHERE estado = ${estado} ORDER BY fecha_hora DESC;
    `
    res.json({ status: 'OK', data: pedidos })
  } catch (error) {
